...
exports.getDiscuss = ( id, page ) => {
    let sql = `select discuss.id, essay_id,headimg,content,p_time,username 
    from discuss,user 
...
*/
//获取文章列表
exports.getArticleByLabeltoAdmin = ( label, page ) => {
    let sql = `select id,title,user_name,label,time,click_num,like_num 
    from list where label="${label}" and caogao=0 order by time desc limit ${(page-1)*20},20;`
    return query( sql )
}
//获取所有用户的列表
exports.getUserList = ( page ) => {
    let sql = `select user_name, user.admin, date from user order by date desc limit ${(page-1)*20}, 20`
    return query( sql )
}
//获取用户总数
exports.getCountUser = () => {
    let count = `select count(*) count from user`
    return query( count )
}
//删除一个评论
exports.deleteDiscuss = ( id ) => {
    let sql = `delete from discuss where id=${ id }`
    return query( sql )
}
